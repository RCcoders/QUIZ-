...
export const setupSocket = (server) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('next_question', async ({ gameCode, nextIndex }) => {
            try {
                if (!socket.user || socket.user.role !== 'teacher')
                    return;
                const session = await GameSession.findOne({ gameCode });
        ...
        socket.on('submit_answer', async ({ gameCode, participantId, answer }) => {
            try {
                ...
                if (participant) {
                    ...
                    if (!session.currentQuestionAnswers)
        ...
        socket.on('reveal_results', async ({ gameCode }) => {
            try {
                if (!socket.user || socket.user.role !== 'teacher')
                    return;
                const session = await GameSession.findOneAndUpdate({ gameCode }, { status: 'results' }, { new: true });
                if (session) {
        ...
        socket.on('end_game', async ({ gameCode }) => {
            try {
                if (!socket.user || socket.user.role !== 'teacher')
                    return;
                const session = await GameSession.findOneAndUpdate({ gameCode }, { status: 'ended', endedAt: new Date() }, { new: true });
                if (session) {
                    const quiz = await Quiz.findById(session.quizId);
                    ...
                    const scorePromises = session.participants.map((p) => {
                        ...
                        return ScoreRecord.create({
                            ...
                            subject: (quiz?.subject || 'General'), // Cast to bypass StringQueryTypeCasting error
