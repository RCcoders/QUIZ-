...
export const generateAdaptiveQuiz = async (
    ...
    if (scores.length > 0) {
        ...
        const averages = Object.entries(difficultyStats).map(([diff, total]) => ({
            diff: diff as 'easy' | 'medium' | 'hard',
            avg: difficultyCounts[diff as keyof typeof difficultyCounts] > 0
                ? total / difficultyCounts[diff as keyof typeof difficultyCounts]
                : 100
        }));
    ...
    const difficultyBreakdown: Record<string, number> = {
        [dominantWeakDifficulty as string]: dominantCount,
        [otherTiers[0] as string]: otherCountEach + remainder,
        [otherTiers[1] as string]: otherCountEach,
    };
...
[
  {
    ...
    try {
        ...
        return questions.sort(
            (a: any, b: any) => (difficultyOrder[a.difficulty] ?? 0) - (difficultyOrder[b.difficulty] ?? 0)
        );
    } catch (error) {
